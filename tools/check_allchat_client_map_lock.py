...
★ЧЕГО ГЕЙТ НЕ УТВЕРЖДАЕТ. Он видит только функции из `MUST_LOCK` и только
текстовые обращения. Вне его поля зрения остаётся, в частности,
`GetClientContext`, отдающий ССЫЛКУ ВНУТРЬ карты: обращения через эту ссылку в
чужих обработчиках формально гоночны против фазы 1 и после раунда. Закрыть класс
целиком — работа размера отдельного раунда (для лобби ею был R64-3, #215). Гейт
стережёт ровно ту границу, которую провёл этот раунд.
...
#: Замер на кандидате R80 — 26 обращений; порог ставится чуть ниже.
MIN_ACCESSES = 22
