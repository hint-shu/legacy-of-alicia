...
ЧТО ИМЕННО ПРОВЕРЯЕТСЯ
  ...
  4. Сам блок обёрнут в `try { … } catch (…) { … } catch (…) { … }` — то есть
     ДВА внешних обработчика на месте, и притом НУЖНЫХ ТИПОВ: первый —
     `catch (const std::exception&)`, последний — `catch (...)`.
...
САМОПРОВЕРКА (гейт сперва доказывает себя)
  `--self-test` строит восемь фикстур из НАСТОЯЩЕГО файла и требует, чтобы гейт
  на каждой упал:
    ...
    фикстура 8 — финальный `catch (...)` заменён на `catch (int)`: обработчиков
                 по-прежнему два, но катч-олла нет — блок снова может бросить
                 (находка ревью, итерация 6, W4);
...
#: LOA (R70-fix-8, находка Codex 6 WARN-4): ★ТИП ОБРАБОТЧИКА, А НЕ ФАКТ ЕГО
#: НАЛИЧИЯ. Прежняя версия считала ДВА синтаксических `catch` и на этом
#: успокаивалась — замена финального `catch (...)` на `catch (int)` проходила
#: гейт насквозь (проверено Codex на живом парсере). Но свойство, которое гейт
#: обязан доказывать, звучит «блок НЕ МОЖЕТ бросить», а его даёт именно
#: КАТЧ-ОЛЛ: `catch (int)` ловит ровно int и возвращает форму дефекта #233.
#: Поэтому проверяются оба типа поимённо: первый — `const std::exception&`
#: (он несёт `.what()` в лог), последний — многоточие.
CATCH_ALL = re.compile(r"catch\s*\(\s*\.\.\.\s*\)")
CATCH_STD_EXCEPTION = re.compile(
    r"catch\s*\(\s*const\s+std\s*::\s*exception\s*&")
...
def check_catches(lines, marker, end):
    ...
    handlers = []
    for position in zeros:
        rest = "".join(ch for (_, _, ch) in chars[position + 1:func_close]).lstrip()
        if CATCH_CLAUSE.match(rest):
            handlers.append(rest)
    if len(handlers) != 2:
        return (
            "у блока достижений в Stop() %d внешних `catch`, а обязано быть два "
            "(`const std::exception&` и `...`): бросок из пролога снова "
            "вылетел бы из Stop() и вызвал повторную выплату" % len(handlers))

    # ★ТИПЫ ПРОВЕРЯЮТСЯ ПОИМЁННО (W4). Считать «две штуки» недостаточно: два
    # обработчика узких типов оставляют блок бросающим, а гейт — зелёным.
    if not CATCH_STD_EXCEPTION.match(handlers[0]):
        return (
            "ПЕРВЫЙ внешний обработчик блока достижений в Stop() — не "
            "`catch (const std::exception&)`: %s"
            % handlers[0].splitlines()[0].strip())
    if not CATCH_ALL.match(handlers[-1]):
        return (
            "ПОСЛЕДНИЙ внешний обработчик блока достижений в Stop() — не "
            "`catch (...)`, то есть блок ВСЁ ЕЩЁ может бросить: %s"
            % handlers[-1].splitlines()[0].strip())
    return None
...
def self_test(path):
    ...
    # Фикстура 8 (W4): финальный `catch (...)` заменён на `catch (int)`.
    # Два синтаксических обработчика на месте, хвост пуст, `try` на месте —
    # красным обязан стать ИМЕННО контроль ТИПА. Мутация построена из
    # НАСТОЯЩЕГО файла: строка ищется внутри блока, а не пишется руками.
    # ★ПОСЛЕДНИЙ `catch (...)` В БЛОКЕ, А НЕ ПЕРВЫЙ: внутри блока есть ВТОРОЙ,
    # пер-гонщиковый, и мутация первого попавшегося ничего не доказывала бы —
    # мутировать надо тот самый внешний катч-олл, на котором держится свойство.
    catch_all_line = next(
        (index for index in reversed(range(marker_lines[0], block_end_line + 1))
         if lines[index].strip() == "catch (...)"), None)
    if catch_all_line is None:
        print("САМОПРОВЕРКА: строка `catch (...)` блока не найдена — фикстуру 8 не построить")
        return 2
    narrowed = list(lines)
    narrowed[catch_all_line] = lines[catch_all_line].replace("catch (...)", "catch (int)")
    fixtures.append(("финальный catch (...) сужен до catch (int)",
                     "\n".join(narrowed) + "\n"))
