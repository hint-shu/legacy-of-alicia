...
//! (256) НИЖЕ бюджета выдачи (4096) — R71-32, ревью 8 #2, — поэтому без снятия записей
//! отказ пришёл бы от потолка ЖИВЫХ и был бы неотличим от отказа по бюджету, а речь
//! здесь именно о бюджете ([[a-blind-checker-says-clean]]). Это и есть тот негатив,
//! который стенд наблюдать НЕ УМЕЕТ: на проводе оба отказа выглядят одинаково.
