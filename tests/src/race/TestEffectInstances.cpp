...
//! пакетами и сразу снимает записи. ★Снимает — не для красоты: потолок ЖИВЫХ записей и
//! бюджет выдачи РАВНЫ (4096, и это правило — см. `RaceTracker.hpp`), поэтому без
//! снятия записей отказ пришёл бы ОДНОВРЕМЕННО по обоим и был бы неотличим, а речь
//! здесь именно о бюджете ([[a-blind-checker-says-clean]]).
