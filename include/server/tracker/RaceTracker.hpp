...
public:
  ...
  //!
  //! ★ПОЧЕМУ ЖИВЫХ МЕНЬШЕ, ЧЕМ ВЫДАННЫХ, И НАСКОЛЬКО ИМЕННО. Записи стены снимаются
  //! (слом или истечение через 4 с), поэтому в живых они почти не накапливаются; а вот
  //! запись НЕ-стены не снимается ничем до конца заезда, и именно она задаёт потолок
  //! живых. Не-стенный каст берёт РОВНО ОДИН номер, значит живых записей у кастера не
  //! больше, чем у него было КАСТОВ, — а кастов честный гонщик делает не больше 1000
  //! (арифметика ниже, у `MaxEffectInstanceIssuancePerRacer`). Берём 1024 — ближайшая
  //! степень двойки выше этого числа.
  //!
  ...
  //! восемью гонщиками — 8 x 1024 x 120 Б ≈ 1 МБ НА КОМНАТУ (прежние 256 давали ≈ 245 КБ).
  //! Рост вчетверо реален и принят сознательно: цена отказа ЧЕСТНОМУ касту выше цены
  //! мегабайта на комнату в предельном случае, которого честная игра не достигает.
  static constexpr size_t MaxEffectInstancesPerRacer = 1024;
