...
public:
  ...
  //!
  //! ★256 — НЕДОСТИЖИМОЕ ЧЕСТНОЕ ЧИСЛО, А НЕ ЗАПАС ПО ВКУСУ: каст требует ПОЛНОЙ
  //! звёздной шкалы (`HandleRequestMagicItem` выдаёт предмет только при
  //! `starPointValue >= starPointsMax` и обнуляет её), поэтому за трёхминутный заезд
  //! честный гонщик кастует единицы раз. Память при полностью выбранном потолке —
  //! восемь гонщиков x 256 записей; запись — это счётчики плюс два коротких вектора
  //! целей (каждый не длиннее `MaxMagicTargetListSize` = 8), то есть десятки
  //! килобайт на комнату, а не мегабайты.
  ...
  //!
  //! ★АРИФМЕТИКА, А НЕ ВКУС. Комната вмещает максимум 8 игроков
  //! (`MaxRoomPlayerCount` в `RaceNetworkHandler.cpp`, кламп стоит и в лобби), значит
  //! одновременных кастеров не больше восьми: 8 x 512 = 4096 номеров, одна
  //! шестнадцатая домена. Даже при роста́ции состава внутри одного заезда домен не
  //! исчерпать раньше, чем 128 РАЗНЫХ oid'ов выберут свой бюджет целиком, — а
  //! `_characterOids` переиспользует oid вернувшегося игрока, то есть перезаход
  //! нового бюджета не даёт. Честному гонщику 512 недостижимы: каст требует полной
  //! звёздной шкалы, а самый «дорогой» каст (ледяная стена) берёт 8 номеров, то есть
  //! 512 — это 64 стены за один заезд.
  //!
  ...
  static constexpr uint32_t MaxEffectInstanceIssuancePerRacer = 512;
  ...
  //!     пока каждый кастер сидит в своём бюджете (8 мест x 512 = 4096 из 65 535).
