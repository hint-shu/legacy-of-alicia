...
public:
  ...
  //! LOA-fix (R71-17, находка ревью 2 #2; расширено R71-20/R71-21 по ревью 3):
  //! ЭКЗЕМПЛЯР ЭФФЕКТА, ВЫДАННЫЙ СЕРВЕРОМ.
  //!
  //! ★ЗАЧЕМ. И слом ледяной стены, и «на мне сработал эффект» объявляет КЛИЕНТ, а
  //! номер экземпляра в его пакете был чисто клиентским. Подсмотрев чужой номер в
  //! `AcCmdCRUseMagicItemNotify` — или просто выдумав его — гонщик ломал чужую стену
  //! и, что хуже, выдавал себе любой зарегистрированный эффект: `targetOid = свой`,
  //! `attackerOid = свой`, `effectId = 2`, `effectInstanceId = 0xBEEF` — и сервер
  //! вешал водяной щит, ни разу не спросив, был ли вообще каст. Реестр даёт серверу
  //! то, чего у него не было: собственную запись о КАЖДОМ выданном экземпляре — какого
  //! он типа и кто его кастовал.
  //!
  //! ★РАНЬШЕ ЗАПИСЫВАЛИСЬ ТОЛЬКО СТЕНЫ — И ЭТО БЫЛ СПИСОК МЕСТ ВМЕСТО ПРАВИЛА
  //! (находка ревью 2 #1). Экземпляры рождаются в ОДНОМ месте (`HandleUseMagicItem`),
  //! поэтому и записываются теперь все, без разбора типа.
  struct EffectInstance
  {
    ...
    //! Щиты, бустеры, разгон и командные бафы вешает `HandleUseMagicItem` в момент
    //! каста. Клиентский отчёт про такой экземпляр — не «на мне сработало», а
    //! повтор чужой работы: в лучшем случае лишняя рассылка, в худшем — выдача
    //! ЧУЖОГО каста себе. Помеченный экземпляр не разрешает НИ ОДНОГО отчёта.
    bool serverApplied{};
    //! LOA-fix (R71-22, находка ревью 3 #1): кого каст НАЗВАЛ целями.
    //! Пусто там, где целей нет по смыслу: у ледяной стены (список — это число
    //! сосулек) и у эффектов, которые сервер применяет сам.
    std::vector<Oid> authorizedTargets{};
    //! LOA-fix (R71-22, находка ревью 3 #1): кто уже отчитался по этому экземпляру.
    //! Один экземпляр — один отчёт НА ЦЕЛЬ; повтор (и «переигровка» тем же номером
    //! после истечения эффекта) отбрасывается ДО всякой рассылки.
    std::vector<Oid> consumedTargets{};
  };

  //! Потолок записей НА ОДНОГО КАСТЕРА за заезд.
  //!
  //! ★ПОЧЕМУ НА КАСТЕРА, А НЕ НА КОМНАТУ (находка ревью 2 #3). Прежний общий потолок
  //! 64 с вытеснением самых старых записей потолком НЕ БЫЛ: он молча предполагал один
  //! каст на гонщика за четыре секунды жизни стены. Девять принятых кастов по восемь
  //! сегментов дают 72 ЖИВЫХ экземпляра, и вытеснение стирало улику о стене, которая
  //! стоит на трассе. Бил этот механизм по ЧЕСТНОЙ игре: у честного гонщика переставала
  //! ломаться его собственная стена, а сломать её было уже нечем. Теперь ёмкость
  //! считается для каждого кастера отдельно — переполнить ЧУЖУЮ улику нельзя в принципе,
  //! и единственный, кому флудер отказывает в кастах, это он сам.
  //!
  //! ★ЖИВЫЕ ЗАПИСИ НЕ ВЫТЕСНЯЮТСЯ НИКОГДА. Их срок жизни — заезд (`Clear()`), а
  //! исчезают они раньше только по событию, которое сервер объявил сам: слом стены или
  //! её истечение. Если места нет — отказывает КАСТ, и отказ стоит ДО списания
  //! крит-бафа, ДО выдачи номеров и ДО любой рассылки (`CanIssueEffectInstances`).
  //!
  //! ★ЧИСЛО ПЕРЕСЧИТАНО ПО ОБОИМ ПУТЯМ ВЫДАЧИ (R71-30, находка ревью 5 #3, WARN).
  //! Прежние 256 стояли на ЛОЖНОМ доводе «каст требует ПОЛНОЙ звёздной шкалы»: шкала
  //! нужна только пути `HandleRequestMagicItem`, а второй путь — подбор магии с трассы
  //! (`HandleUserRaceItemGet`) — шкалы не спрашивает вовсе. Довод был неверен, а
  //! значит и число под ним не значило ничего ([[a-blind-checker-says-clean]]).
  //!
  //! ★ПОТОЛОК ЖИВЫХ ВОЗВРАЩЁН К 256 (R71-32, находка ревью 8 #2, BLOCK) — И ЭТО
  //! ОТМЕНА МОЕГО ЖЕ «ПРАВИЛА ПОРЯДКА» ИЗ ИТЕРАЦИИ 8.
  //!
  //! ★ЧЕМ БЫЛО НЕВЕРНО ПРАВИЛО. Итерация 8 подняла этот потолок 256 -> 4096 доводом
  //! «живых никогда не больше, чем выданных, значит ограничивать обязан бюджет».
  //! Импликация верна, а ВЫВОД из неё — нет: `live <= issued` НЕ означает, что
  //! потолки обязаны быть РАВНЫ. Более низкий потолок живых имеет право остановить
  //! злоупотребление раньше бюджета — это его работа, а не дефект.
  //!
  //! ★ЧЕМ ПОДЪЁМ БЫЛ ПЛОХ ПО ЦЕНЕ. Реестр экземпляров — ВЕКТОР НА КОМНАТУ, и он
  //! сканируется ЛИНЕЙНО под глобальным `_raceInstancesMutex` (`FindEffectInstance`,
  //! списание, снятие, обе проверки ёмкости; `RaceTracker.cpp:387`). Подняв потолок в
  //! 16 раз, я в 16 раз поднял цену ОДНОГО подделанного пакета — и платят её ВСЕ
  //! комнаты, потому что замок один. То есть фикс против заливки диска сам стал
  //! алгоритмическим отказом в обслуживании ([[ask-what-the-new-entity-costs]]:
  //! спрашивал «куда пролезет», не спрашивал «что потребляет»).
  //!
  //! ★И ОТДЕЛЬНО — ПРО МОТИВ. Поднять потолок пришлось, чтобы предикат СТЕНДА
  //! («упёрся ли кастер в бюджет») стал наблюдаем. Менять продакшн-число ради
  //! наблюдаемости собственной проверки — это ложно-зелёный наизнанку
  //! ([[my-own-checks-go-false-green]]): проверку обязан чинить стенд, а не код.
  //! Стенд итерации 9 спрашивает то же самое, ничего не расширяя: кастер обязан
  //! получить ОТКАЗ на исчерпании (какой бы из двух потолков ни связал первым) и
  //! обязан никогда не превысить бюджет выдачи.
  //!
  //! ★ЦЕНА ПАМЯТИ ВОЗВРАЩЕНА ТУДА ЖЕ: запись <= ~120 Б, 8 x 256 x 120 Б ~ 245 КБ на
  //! комнату (у редакции с 4096 было ~3,9 МБ).
  static constexpr size_t MaxEffectInstancesPerRacer = 256;

  //! LOA-fix (R71-25, находка ревью 4 #2): СУММАРНАЯ ВЫДАЧА НОМЕРОВ НА КАСТЕРА ЗА ЗАЕЗД.
  //!
  //! ★ЧТО БЫЛО СЛОМАНО. Ревью 3 закрыло оборот счётчика единственным вопросом «не
  //! переступит ли `_nextEffectInstanceId + count` границу домена» — и это запрет
  //! ПО КОМНАТЕ. Счётчик один на заезд, поэтому один кастер, наполняя калибр
  //! клиентским `gainedStarPoints` и тут же ломая свои же стены (живые записи
  //! освобождаются), выжигал общее пространство номеров, и с этого момента КАЖДЫЙ
  //! честный гонщик получал отказ в касте до конца заезда. Фикс безопасности стал
  //! DoS'ом по честной игре — ровно тот класс, ради которого раунд и затеян.
  //!
  //! ★ПОЧЕМУ БЮДЖЕТ, А НЕ ОБЩИЙ ПОТОЛОК. Бюджет пер-кастерный: исчерпав его, гонщик
  //! отказывает в кастах ТОЛЬКО СЕБЕ, а соседи продолжают кастовать. Ровно так же
  //! устроен потолок ЖИВЫХ записей выше — два ограничения, но одно правило: цена
  //! флуда достаётся флудеру.
  //!
  //! ★АРИФМЕТИКА ПЕРЕСЧИТАНА ПО ФАКТИЧЕСКИМ ЧИСЛАМ (R71-32, находка ревью 8 #3, WARN).
  //!
  //! ★ЧТО БЫЛО НЕВЕРНО В ПРЕДЫДУЩЕМ ВЫВОДЕ, названо поимённо: `starPointsMax` в
  //! Magic-режиме равен 100 000, а не 120 000 (120 000 — это Speed и Tutorial,
  //! `courses.yaml`); «золотая подкова с `respawnTime` 500 мс» — дека 101/102, то есть
  //! СКОРОСТНОГО режима, а магический режим спавнит только деки 201/202/203 по 3000 мс;
  //! и главное — пассивная регенерация шкалы (`RaceInstance::TickMagicGauge`) не
  //! учитывалась ВООБЩЕ, хотя именно она и есть основной источник калибра. Вывод,
  //! опирающийся на числа из соседнего режима, — это перенесённая форма
  //! ([[transplanted-fix-shape-is-plausible-and-wrong]]).
  //!
  //! ★★ЧТО ЭТО ЧИСЛО ЕСТЬ, СКАЗАНО ДО АРИФМЕТИКИ (ревью 9 #1, WARN). 4096 — это
  //! ПОЛИТИЧЕСКАЯ ОЦЕНКА по СЕГОДНЯШНЕЙ, проверяемой `config_drift.sh` конфигурации
  //! плюс явно перечисленные допущения о честном клиенте. Это НЕ доказанный сервером
  //! потолок: `minStatSum` = 230 — порог ступени прогрессии, а не максимум смелости;
  //! «courage <= 280» и «около одного препятствия в секунду» — допущения, а не
  //! ограничения кода; а `pointPerTick`, `intervalMs`, `respawnTime` и `timeLimit`
  //! лежат bind-mount'ом и кодом не зажаты вовсе. Меняется конфиг — пересчитывать
  //! надо и это число ([[gate-by-form-gives-false-completeness]]).
  //!
  //! ★ОЦЕНКА СЧИТАЕТСЯ ОТ ТОГО, ЧТО СЕРВЕР ОГРАНИЧИВАЕТ САМ, И ОТ НАЗВАННЫХ ДОПУЩЕНИЙ:
  //!
  //!  1. гонщик держит РОВНО ОДИН магический предмет за раз (`racer.magicItem`
  //!     спрашивается и на выдаче `HandleRequestMagicItem`, и на касте), поэтому
  //!     КАСТОВ НЕ БОЛЬШЕ, ЧЕМ ПОЛУЧЕНИЙ ПРЕДМЕТА;
  //!  2. путь ПОДБОРА (`HandleUserRaceItemGet`, ветка `GameMode::Magic`): кулдаун на
  //!     пару «гонщик + экземпляр деки» = `deck.respawnTime` = 3000 мс у всех трёх
  //!     магических дек, а экземпляров магической деки на блоке карты не больше ТРЁХ
  //!     (максимум по всем 55 блокам `courses.yaml`) → не больше 1 получения в секунду;
  //!  3. путь ШКАЛЫ (`HandleRequestMagicItem`): нужен полный калибр
  //!     `starPointsMax` = 100 000, и выдача обнуляет шкалу (`racer.starPointValue = 0`).
  //!     Самый быстрый ЧЕСТНЫЙ налив — пассивный тик `TickMagicGauge`:
  //!     `pointPerTick` = 1000 за `intervalMs` = 200 мс (`magic.yaml`), с самым
  //!     выгодным множителем смелости `(1000 + 9 * courage) / 1000` — ДОПУЩЕНИЕ (кодом
  //!     не проверяется): сумму статов лошади считаем не выше `minStatSum` последней
  //!     ступени прогрессии (230), откуда берём courage <= 280 с запасом на бонус
  //!     снаряжения → <= 3,52x; плюс сет-эффект 2
  //!     (`passiveGaugeScaleBp` 3300) → x1,33; плюс баф BufGauge → x2. Итого не больше
  //!     ~9 364 за тик = ~46 800 в секунду. Сверху — награды за препятствия
  //!     (`perfectJumpStarPoints` 10 000 + `perfectJumpUnitStarPoints` 1000 x комбо 1),
  //!     не чаще ~1 препятствия в секунду на честной трассе → +11 000/с. Итого
  //!     <= ~57 800/с, то есть полный калибр не быстрее чем за ~1,73 с →
  //!     не больше ~0,58 каста в секунду;
  //!  4. длительность заезда: `timeLimit` в поставляемом `courses.yaml` не больше
  //!     300 с; берём ВДВОЕ больший запас — 600 с (конфиг лежит bind-mount'ом);
  //!  5. номеров на каст: не больше ТРЁХ (не-стена берёт ровно один, стена — 1 или 3,
  //!     форма проверена гардом R71-28).
  //!
  //! Итого оценка честной игры: (1,0 + 0,58) x 600 x 3 = ~2 844 номера за заезд.
  //! Берём **4096** — ближайшая степень двойки выше оценки.
  //!
  //! ★ЧЕГО ЭТА ОЦЕНКА НЕ ДОКАЗЫВАЕТ, СКАЗАНО ВСЛУХ. Клиентский путь
  //! `HandleStarPointGet` кладёт `command.gainedStarPoints` в шкалу без серверной
  //! проверки величины (`RaceNetworkHandler.cpp:3641`), поэтому НАПОЛНЕНИЕ
  //! ШКАЛЫ СЕРВЕРОМ НЕ ОГРАНИЧЕНО, и число выше — ПОЛИТИЧЕСКАЯ ОТСЕЧКА, посчитанная
  //! по названным допущениям, а не доказанная граница возможного и не проверяемое
  //! кодом свойство честной игры. Именно поэтому бюджет вообще существует: он —
  //! выбранная нами отсечка, а не вывод из доказанных ограничений. Сам ungated-путь —
  //! отдельная находка (ревью 4 #5, остаётся OPEN), и закрывать её этим числом
  //! нельзя ([[a-check-nobody-reads-is-not-a-check]]).
  //!
  //! ★ЗАПРЕТ ОБОРОТА ОТ ЭТОГО НЕ ОСЛАБ. Восемь мест x 4096 = 32 768 номеров, половина
  //! шестнадцатибитного домена (65 535); `_characterOids` переиспользует oid
  //! вернувшегося игрока, поэтому перезаход нового бюджета не даёт. Фейл-клоузный
  //! вопрос о границе домена ниже (`CanIssueEffectInstances`) — рабочий backstop.
  //!
  //! ★ПОТОЛОК ЖИВЫХ (256) НИЖЕ ЭТОГО БЮДЖЕТА, И ЭТО НОРМАЛЬНО: он связывает раньше,
  //! то есть злоупотребление останавливается раньше. ★ЧЕГО ЗДЕСЬ НЕ УТВЕРЖДАЕТСЯ
  //! (ревью 9 #2): что честная игра 256 живых записей НЕ ДОСТИГАЕТ. Запись НЕ-стенного
  //! каста не снимается до конца заезда, поэтому достижимость 256 зависит от тех же
  //! недоказанных допущений о темпе получения магии, что и оценка выше. Если стенд
  //! или прод однажды покажут отказ честному кастеру — поднимать надо ЭТОТ потолок и
  //! только вместе с заменой линейного скана на индекс (см. backlog), а не «на глаз».
  static constexpr uint32_t MaxEffectInstanceIssuancePerRacer = 4096;

  //! Хватит ли места записать `count` новых экземпляров этого кастера.
  //! ★Спрашивать ОБЯЗАТЕЛЬНО ДО выдачи номеров: отказ должен случиться раньше любого
  //! необратимого шага, иначе каст «состоялся», а улики о нём нет.
  //!
  //! ★ЗДЕСЬ ЖЕ ЖИВЁТ ЗАПРЕТ ОБОРОТА (находка ревью 3 #5). Прежняя редакция считала
  //! только ЖИВЫЕ записи, а живая запись освобождается по слому и по истечению
  //! стены — то есть СУММАРНАЯ выдача номеров не была ограничена ничем, счётчик
  //! доходил до оборота, и предикат `HasIssuedEffectInstanceId` после оборота
  //! принимал ВЕСЬ домен.
  //!
  //! ★ВОПРОСОВ ТЕПЕРЬ ТРИ, И ПОРЯДОК У НИХ НЕ СЛУЧАЙНЫЙ (находка ревью 4 #2):
  //!  1. бюджет ВЫДАЧИ этого кастера (`MaxEffectInstanceIssuancePerRacer`) —
  //!     единственный достижимый отказ, и он локален: кастер отказывает себе;
  //!  2. шестнадцатибитная граница домена — фейл-клоузный backstop, недостижимый,
  //!     пока каждый кастер сидит в своём бюджете (8 мест x 4096 = 32 768 из 65 535).
  //!     ★Граница строгая и без запаса: последний допустимый номер — 0xFFFE, чтобы
  //!     `_nextEffectInstanceId` после инкремента остался представимым и НИКОГДА не
  //!     стал нулём;
  //!  3. место под улику — потолок ЖИВЫХ записей этого кастера
  //!     (`MaxEffectInstancesPerRacer`).
  //!
  //! ★КОНТРАКТ ПАКЕТА (R71-26, находка ревью 6 «двойной вопрос о бюджете»): на ОДИН
  //! пакет каста этот вопрос задаётся РОВНО ОДИН РАЗ и на ВЕСЬ `count`, и сразу за
  //! ним идёт единственное списание (`GetNextEffectInstanceIdAndIncrementBy`).
  //! Ниже по течению бюджет НЕ ПЕРЕСПРАШИВАЕТСЯ: после списания `alreadyIssued` уже
  //! включает этот пакет, поэтому повторный вопрос «влезет ли ещё один» отвечает
  //! «нет» ровно на честном ПОГРАНИЧНОМ касте — том, что доводит кастера ровно до
  //! потолка. Так и было: номер выдавался, баф списывался, каст рассылался, а
  //! записи не создавались — и собственный честный отчёт кастера получал отказ
  //! ([[a-check-nobody-reads-is-not-a-check]] наоборот: проверка, которая
  //! СРАБАТЫВАЕТ там, где не должна). Резервируй-и-фиксируй: спросили на весь пакет
  //! → списали весь пакет → выдали номера → записали записи без новых вопросов.
  [[nodiscard]] bool CanIssueEffectInstances(Oid casterOid, uint16_t count) const;
  //! Записывает выданные сервером экземпляры (`count` подряд с `firstInstanceId`).
  //! ★Бюджет выдачи здесь НЕ ПРОВЕРЯЕТСЯ И НЕ СПИСЫВАЕТСЯ (R71-26): и то и другое
  //! уже случилось выше по течению, ровно один раз на пакет. Единственный оставшийся
  //! здесь фейл-клоуз — место под ЖИВУЮ запись (`HasRoomForOneLiveEffectInstance`):
  //! он не зависит от списания, поэтому переспросить его безопасно.
  //! @param serverApplied Сервер уже повесил этот эффект сам (отчёты запрещены).
  //! @param authorizedTargets Кого каст назвал целями (пусто — целей нет по смыслу).
  ...
  //! @returns Указатель на запись либо `nullptr`.
  [[nodiscard]] const EffectInstance* FindEffectInstance(uint16_t instanceId) const;
  //! LOA-fix (R71-22, находка ревью 3 #1): ПОТРЕБЛЕНИЕ ПАРЫ «ЭКЗЕМПЛЯР + ЦЕЛЬ».
  //!
  //! Отмечает, что цель `targetOid` отчиталась по экземпляру `instanceId`.
  //! @retval `true`  отчёт первый — можно применять эффект
  //! @retval `false` экземпляра нет ЛИБО эта цель по нему уже отчитывалась
  //!
  //! ★Звать ПОСЛЕДНИМ из гардов и ДО любого побочного действия: это и проверка, и
  //! необратимая пометка одновременно ([[muffling-a-throw-is-not-a-fix]] — состояние
  //! обязано меняться ровно тогда, когда проверка пройдена).
  [[nodiscard]] bool ConsumeEffectInstanceTarget(uint16_t instanceId, Oid targetOid);
  //! LOA-fix (R71-22): фейл-клоуз для сверки классификации в `HandleUseMagicItem`.
  //! Помечает экземпляр как уже применённый сервером — то есть запрещает по нему
  //! любые клиентские отчёты. Молчит, если экземпляра нет.
  void MarkEffectInstanceServerApplied(uint16_t instanceId);
  //! Снимает экземпляр (сломан или истёк). Молчит, если его уже нет.
  void RemoveEffectInstance(uint16_t instanceId);
  //! Снимает `count` экземпляров подряд — форма, в которой они выдавались.
  void RemoveEffectInstances(uint16_t firstInstanceId, uint16_t count);
  ...
  //! Выдаёт `increment` подряд идущих номеров и СПИСЫВАЕТ их с бюджета кастера.
  //! ★Кастер здесь обязателен именно затем, чтобы списание нельзя было забыть: это
  //! единственная точка выдачи, и она же единственная точка учёта
  //! ([[total-invariant-beats-list-of-sites]]).
  uint16_t GetNextEffectInstanceIdAndIncrementBy(Oid casterOid, uint16_t increment);

  //! LOA-fix (R71-15, находка ревью 2 #3): ВЫДАВАЛ ЛИ СЕРВЕР ТАКОЙ ИДЕНТИФИКАТОР
  //! ЭКЗЕМПЛЯРА ЭФФЕКТА.
  //!
  //! Отвечает ровно на то, что сервер знает наверняка, и ни на слово больше:
  //! идентификаторы выдаёт ТОЛЬКО `GetNextEffectInstanceIdAndIncrementBy`, счётчик
  //! шестнадцатибитный и монотонный, поэтому «выдан» = «меньше счётчика».
  //! Тридцатидвухбитное число с провода, не влезающее в домен, отвергается по
  //! построению.
  //!
  //! ★ФЛАГА ОБОРОТА БОЛЬШЕ НЕТ, И ЭТО ГЛАВНОЕ (находка ревью 3 #5). Он существовал
  //! ради честного клиента после переполнения счётчика и превращал предикат в
  //! «всегда да» — то есть гард оставался на месте и переставал что-либо значить.
  //! Ревью 2 закрыло это арифметикой («потолок x восемь мест меньше домена»), но
  //! арифметика считала ЖИВЫЕ записи, а живая запись освобождается по слому и по
  //! истечению стены: ломая свои же стены, кастер выдавал номера дальше без предела.
  //! Теперь оборот запрещён В ИСТОЧНИКЕ — `CanIssueEffectInstances` не даёт выдать
  //! номер, переступающий шестнадцатибитную границу, — и предикат снова говорит
  //! правду. ★Если запрет когда-нибудь обойдут, отказ будет ЗАКРЫТЫМ, а не
  //! открытым: после оборота счётчик мал и `id < _nextEffectInstanceId` отвергает
  //! почти весь домен вместо того, чтобы принимать его целиком.
  //!
  //! ★ЧЕГО ЭТО НЕ УТВЕРЖДАЕТ, ЧИТАТЬ ОБЯЗАТЕЛЬНО: что экземпляр ЖИВ и что он
  //! принадлежит спрашивающему. Сильная проверка — «экземпляр в реестре, того же типа
  //! и от названного кастера» — стоит там, где сервер знает семантику
  //! (`FindEffectInstance` в `HandleActivateSkillEffect`). Здесь же речь о поле
  //! ретрансляции, которое сервер не интерпретирует вовсе; сузить его до живого
  //! экземпляра значило бы выбрасывать честные кадры, у которых просто истёк
  //! названный эффект.
  [[nodiscard]] bool HasIssuedEffectInstanceId(uint32_t effectInstanceId) const;

  void Clear();

private:
  //! LOA-fix (R71-26, находка ревью 6): ЕСТЬ ЛИ МЕСТО ПОД ЕЩЁ ОДНУ ЖИВУЮ ЗАПИСЬ.
  //! Спрашивает ТОЛЬКО про потолок живых записей кастера и НИЧЕГО про бюджет выдачи —
  //! именно поэтому его можно звать после списания, не получая ложного отказа на
  //! пограничном касте.
  [[nodiscard]] bool HasRoomForOneLiveEffectInstance(Oid casterOid) const;
  ...
  //! ★Пер-заездный (`Clear()`) и НИКОГДА не оборачивается: выдачу за границу домена
  //! запрещает `CanIssueEffectInstances` (R71-22, находка ревью 3 #5).
  uint16_t _nextEffectInstanceId = 0;
  //! LOA-fix (R71-17/R71-20): живые экземпляры эффектов этого заезда — ВСЕ, а не
  //! только ледяные стены.
  std::vector<EffectInstance> _effectInstances;
  //! LOA-fix (R71-25, находка ревью 4 #2): сколько номеров каждый кастер уже выбрал
  //! за ЭТОТ заезд. Пер-заездное (`Clear()`), как и сам счётчик номеров.
  std::unordered_map<Oid, uint32_t> _issuedEffectInstanceCounts;
};
