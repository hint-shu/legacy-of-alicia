...
public:
  ...
  //!     выгодным множителем смелости `(1000 + 9 * courage) / 1000` — сумма статов
  //!     лошади ограничена `minStatSum` последней ступени прогрессии (230), берём
  //!     courage <= 280 с запасом на бонус снаряжения → <= 3,52x; плюс сет-эффект 2
  ...
  //! ШКАЛЫ СЕРВЕРОМ НЕ ОГРАНИЧЕНО и число выше — граница ЧЕСТНОЙ игры, а не
  //! доказанная граница возможного. Именно поэтому бюджет вообще существует: он и
  //! есть тот предел, за которым выдача перестаёт быть честной. Сам ungated-путь —
  ...
  //! недоказанных допущений о темпе получения магии, что и оценка ниже. Если стенд
