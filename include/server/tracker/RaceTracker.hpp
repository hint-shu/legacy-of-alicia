...
public:
  ...
  //! восемь гонщиков x 256 записей x 8 байт ~ 16 КБ на комнату.
  ...
  //! шестнадцатибитный и монотонный, поэтому «выдан» = «меньше счётчика» (а после
  //! переполнения — весь домен). Тридцатидвухбитное число с провода, не влезающее
  //! в домен, отвергается по построению.
  //!
  //! ★ОБОРОТ БОЛЬШЕ НЕДОСТИЖИМ, И ЭТО ВАЖНО (собственная находка при разборе ревью 2
  //! #3): счётчик и флаг оборота НЕ чистились в `Clear()`, а трекер живёт вместе с
  //! КОМНАТОЙ, а не с заездом. Значит после ~65 535 выданных номеров за жизнь комнаты
  //! этот предикат деградировал в «всегда да» до самого закрытия комнаты — гард
  //! оставался на месте и переставал что-либо значить. Теперь номера пер-заездные
  //! (`Clear()` сбрасывает счётчик, как давно сбрасывает `_nextItemDeckOid`), а
  //! выдача за заезд ограничена сверху потолком `MaxEffectInstancesPerRacer` на
  //! восемь мест — 16 384 номера, вчетверо меньше домена.
  //!
...
private:
  ...
  //! LOA-fix (R71-15): было ли переполнение счётчика экземпляров. Без флага
