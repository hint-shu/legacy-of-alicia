...
public:
  ...
  struct Racer
  {
    ...
    //!   (1) гонщик честно объявил СХОД (`raceTrackProgress > 0` в
    //!       `AcCmdUserRaceFinal`) — это и есть «сход» оригинала
    //!       (`ach_conditions.lua`, `Retire`: `TimeRecord == 0`);
    ...
    enum class FinishOutcome
    {
      ...
      Finished,
      //! Заявил СХОД. Единственное значение, которое достижения считают сходом.
      Retired,
      //! Заявил финиш, античит его отверг. Сходом НЕ является.
      Rejected,
    };
