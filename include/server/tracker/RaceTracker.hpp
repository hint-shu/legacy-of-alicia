...
public:
  ...
  //!
  //! ★ПОЧЕМУ ПОТОЛОК ЖИВЫХ РАВЕН БЮДЖЕТУ ВЫДАЧИ. Запись стены снимается по слому и по
  //! истечению, а запись НЕ-стены не снимается ничем до конца заезда. Значит честный
  //! гонщик, кастующий весь заезд, доводит число ЖИВЫХ записей ровно до числа выданных
  //! номеров. Держать потолок живых НИЖЕ бюджета выдачи означало бы отказывать честному
  //! касту раньше, чем исчерпан бюджет, — тот самый «фикс безопасности стал DoS'ом по
  //! честной игре», ради которого раунд и затеян. Поэтому оба числа выведены из ОДНОЙ
  //! арифметики и равны (см. `MaxEffectInstanceIssuancePerRacer` ниже).
  //!
  ...
  //!
  //! ★ЧЕСТНЫЙ ПОТОЛОК СЧИТАЕТСЯ ОТ ТОГО, ЧТО СЕРВЕР ОГРАНИЧИВАЕТ САМ:
  //!
  ...
  //!
  //! Итого честный потолок: (1,0 + 0,58) x 600 x 3 = ~2 844 номера за заезд.
  //! Берём **4096** — ближайшая степень двойки выше честного потолка.
  //!
  //! ★ЧЕГО ЭТОТ ВЫВОД НЕ ДОКАЗЫВАЕТ, СКАЗАНО ВСЛУХ. Клиентский путь
  //! `HandleUserRaceHurdleClearResult` кладёт `command.gainedStarPoints` в шкалу без
  //! серверной проверки величины (`RaceNetworkHandler.cpp:3641`), поэтому НАПОЛНЕНИЕ
  ...
  //! значит злоупотребление останавливается раньше, а честная игра его не достигает:
  //! живая запись снимается по истечении эффекта, и 256 ОДНОВРЕМЕННО живых эффектов
  //! одного кастера честным путём не набираются.
