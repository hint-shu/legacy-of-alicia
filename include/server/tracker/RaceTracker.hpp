...
public:
  ...
  //! принадлежит спрашивающему. Для ледяной стены такая — сильная — проверка есть
  //! отдельно (`ConsumeIceWallInstance`). Здесь же речь о поле, которое сервер не
  //! интерпретирует вовсе, и придумывать ему семантику значило бы выдумывать
  //! правило, а не проверять его.
