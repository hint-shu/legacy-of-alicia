...
//! LOA-fix (R81, backlog #270): ПОТОЛОК ЧИСЛА ПЛАТНЫХ РЫВКОВ ЗА ЗАЕЗД.
//! ★ОТКУДА ЧИСЛО — тот же вывод, что у `MaxPlausibleBoostChain` выше, и он был
//! сделан именно «за ВЕСЬ заезд»: рывок стоит 40 000, барьер даёт не больше
//! 12 000 + 5×1000 = 17 000 (courses.yaml, type 1), значит на рывок нужно >= 3
//! барьера; круг в 300 с с барьером каждые ~2 с даёт <= 150 барьеров => <= 50
//! честных рывков. Цепочка (`MaxPlausibleBoostChain`) — непрерывная ЧАСТЬ этого
//! же числа, поэтому потолки равны, и это не совпадение.
constexpr uint32_t MaxPlausibleSpursPerRace = 50;

//! LOA-fix (R81, backlog #270): СКОЛЬКО РАЗ ЗА ЗАЕЗД СЕРВЕР ПРОЩАЕТ НЕХВАТКУ.
//! ★ЧЕСТНО: ЭТО ПОРЯДОК ЕДИНИЦ, А НЕ ВЫВЕДЕННАЯ ВЕЛИЧИНА. Ни одного замера
//! величины и частоты нехватки у нас нет — код до R81 бросал, не называя чисел.
//! 8 выбрано так, чтобы (а) покрыть наблюдаемое (по одному отбитому рывку на
//! заезд в двух окнах тестера) с запасом в разы и (б) остаться заведомо ниже
//! потолка рывков за заезд. Строка лога, которую заводит этот же раунд, даёт
//! распределение нехватки — калибровать ВТОРЫМ проходом.
constexpr uint32_t SpurShortfallGrace = 8;

//! LOA-fix (R81, backlog #273): ПОТОЛОК ОДНОГО ОТРЕЗКА СКОЛЬЖЕНИЯ.
//! ★ОТКУДА ЧИСЛО. Занос — манёвр поворота, а не режим движения: круг длится
//! 90-180 с (тот же вывод, что у `BoostComboWindow`), и непрерывный занос в
//! 30 с — это пятая часть круга боком.
constexpr std::chrono::milliseconds MaxPlausibleSlideDuration{30000};

//! LOA-fix (R81, backlog #273): ПОТОЛОК СУММАРНОГО СКОЛЬЖЕНИЯ ЗА ЗАЕЗД —
//! ДОЛЯ ВРЕМЕНИ ЗАЕЗДА. ★ЭТО НЕ ТО ЖЕ САМОЕ, ЧТО ПОТОЛОК ОДНОГО ОТРЕЗКА.
//! ★ПОЧЕМУ НЕ `courseTime` ЦЕЛИКОМ. «Скользить дольше, чем ехал, нельзя»
//! отбрасывает только ФИЗИЧЕСКИ НЕВОЗМОЖНОЕ. Десять закрытых отрезков по 30 с
//! в 300-секундном заезде проходят пер-отрезковый потолок и кладут ~300 единиц
//! в ВЕЧНОЕ МОНОТОННОЕ поле, знаменатель панели которого 900
//! (progression.yaml) — полоса заполняется за три-четыре заезда, а снять
//! записанное можно только правкой файлов при ОСТАНОВЛЕННОМ сервере.
//! ★ОТКУДА 25 %. Соседний вечный потолок того же семейства
//! (`MaxPlausibleGlideMetres`, R75) выбран как 29 % круга. Наблюдение с другой
//! стороны: T76 дал панели 15 единиц за 4 заезда, то есть ~4 с скольжения на
//! ~150-секундный заезд = ~2,7 % времени. 25 % — это ДЕВЯТИКРАТНЫЙ запас над
//! единственным имеющимся замером и тот же порядок строгости, что у глайда.
//! ★ПОДНИМАТЬ НЕЛЬЗЯ. Поле вечное и монотонное; ослабление потолка необратимо
//! для всех коней, успевших записать раздутое значение. Сужать — можно и нужно,
//! как только аудит-строка `slide {} ms` даст распределение с живого прода.
constexpr uint32_t MaxPlausibleSlideShareOfCoursePercent = 25;

//! LOA-fix (R81, backlog #273): ПОТОЛОК УДАЧНЫХ ПРЫЖКОВ ЗА ЗАЕЗД.
//! ★СВОЙ ВЫВОД, А НЕ ССЫЛКА НА СОСЕДНИЙ. Верхняя граница ПРЕДЪЯВЛЕННЫХ
//! барьеров на круге — 150 (круг 300 с, барьер каждые ~2 с; тот же счёт, что у
//! `MaxPlausibleBoostChain`). Счётчик считает только УСПЕШНЫЕ (Perfect + Good),
//! а успешных не может быть больше предъявленных — значит 150 остаётся
//! корректной верхней границей и для него, просто с запасом. Наблюдение: T76
//! дал 96 единиц панели прыжков за 4 заезда, то есть ~24 за заезд — запас
//! шестикратный.
constexpr uint32_t MaxPlausibleJumpsPerRace = 150;

//! LOA-fix (R81, backlog #273): ПОТОЛОК СУММЫ «РЫВКИ + МАГИЧЕСКИЕ ПРЕДМЕТЫ».
//! ★ПОЧЕМУ ОТДЕЛЬНАЯ КОНСТАНТА, ХОТЯ ЧИСЛО БЛИЗКО К СОСЕДНЕМУ.
//! `MaxPlausibleSpursPerRace` выведен из экономики РЫВКОВ; этот счётчик
//! суммирует рывки И выданные предметы, и мерить его чужим выводом значило бы
//! ложно срабатывать на честном МАГИЧЕСКОМ заезде — а раунд не клампит, он
//! ОТБРАСЫВАЕТ всё приращение.
//! ★СОБСТВЕННЫЙ ВЫВОД. Оба события расходуют ОДНУ И ТУ ЖЕ шкалу, а её доход за
//! заезд ограничен барьерами: <= 150 × 17 000 (Speed) = 2 550 000 и
//! <= 150 × 11 000 (Magic) = 1 650 000. Самое ДЕШЁВОЕ событие — рывок, 40 000.
//! Значит честных событий не больше 2 550 000/40 000 = 63 в Speed и
//! 1 650 000/40 000 = 41 в Magic. Берём 64 — ближайшую степень двойки над
//! худшим случаем: потолок обязан НЕ срабатывать на честной игре, его задача —
//! ограничить РАЗМЕР лжи, а не поймать её.
constexpr uint32_t MaxPlausibleSpurMagicPerRace = 64;
...
public:
  ...
  struct Racer
  {
    ...
    //! ★ЭТИ ПОЛЯ СТОЯТ В КОНЦЕ СТРУКТУРЫ ПО ТОЙ ЖЕ ПРИЧИНЕ, ЧТО И БЛОК R75
    //! выше: Racer читают функции, которые раунд не правит, и одна из них
    //! служит НЕРАСТУЩИМ КОНТРОЛЕМ лесенки. Вставка в середину сдвинула бы
    //! смещения полей после точки вставки — контроль «вырос» бы, не изменившись
    //! ни на строку. Не переносить выше «ради группировки».
    //! Живут РОВНО один заезд; обнуляются в `HandleStartRace`, и это стережёт
    //! гейт `tools/round/check_track_journal.py` (раунд добавил маркер этого
    //! блока в его список и научил его видеть конец структуры).
    //! ПОТОКИ: под `_raceInstancesMutex`, как весь трекер.

    //! #270: сколько платных рывков засчитано за заезд.
    uint32_t paidSpurCount{};
    //! #270: сколько нехваток шкалы прощено за заезд.
    uint32_t toleratedSpurShortfalls{};
    //! #273: рывки + выданные магические предметы за заезд.
    uint32_t raceSpurMagicCount{};
    //! #273: удачные прыжки за заезд (Perfect + Good).
    uint32_t raceJumpCount{};
    //! #273: суммарное ЗАКРЫТОЕ скольжение за заезд, мс.
    uint32_t slidingMillis{};
    //! #273: открыт ли отрезок скольжения прямо сейчас.
    bool isSliding{false};
    //! #273: момент открытия отрезка. max() = отрезка нет.
    std::chrono::steady_clock::time_point slidingSince{
      std::chrono::steady_clock::time_point::max()};
    //! #273: ИЗМЕРЕНИЕ планирования за заезд, метры. В лошадь НЕ пишется.
    double raceGlideMetres{};
  };
