...
public:
  ...
  //!
  //! ★ПОТОЛОК ЖИВЫХ НЕ ИМЕЕТ ПРАВА БЫТЬ НИЖЕ БЮДЖЕТА ВЫДАЧИ, И ЭТО ПРАВИЛО, А НЕ ВКУС.
  //! Живая запись рождается только вместе с выданным номером и никогда не появляется
  //! без него, поэтому живых у кастера НИКОГДА не больше, чем выданных. Значит из двух
  //! потолков ограничивать обязан ТОТ, У КОТОРОГО ЕСТЬ ЧЕСТНЫЙ ВЫВОД, — бюджет выдачи
  //! (арифметика ниже). Поставь потолок живых ниже — и связывать начнёт он, а его
  //! честного вывода не существует: не-стенный каст берёт РОВНО ОДИН номер и оставляет
  //! РОВНО ОДНУ запись, которая до конца заезда не снимается, поэтому гонщик, кастующий
  //! одни атаки, упёрся бы в него ВДВОЕ раньше бюджета. Это ровно тот класс «фикс
  //! безопасности стал DoS'ом по честной игре», ради которого раунд и затеян
  //! ([[dont-trade-success-path-for-failure-path]]).
  //!
  //! ★ПОЭТОМУ ОБА ЧИСЛА РАВНЫ, и потолок живых остаётся тем, чем и был, — ФЕЙЛ-КЛОУЗНЫМ
  //! BACKSTOP'ом на случай, если однажды разойдутся пути выдачи и записи. Проверка,
  //! которую нельзя достичь, дешевле проверки, которой нет.
  //!
  //! ★ЧЕМ ЭТО ПЛАТИТСЯ, НАЗВАНО ВСЛУХ ([[ask-what-the-new-entity-costs]]). Запись — это
  //! ~56 Б полей плюс два вектора целей, каждый не длиннее `MaxMagicTargetListSize` = 8
  //! (до ~64 Б кучи), то есть ≤ ~120 Б в худшем случае. Полностью выбранный потолок
  //! восемью гонщиками — 8 x 4096 x 120 Б ≈ 3,9 МБ НА КОМНАТУ (прежние 256 давали
  //! ≈ 245 КБ). Рост реален и принят сознательно: он достигается только умышленным
  //! выкручиванием обоих потолков сразу, а цена ОТКАЗА ЧЕСТНОМУ КАСТУ выше цены
  //! четырёх мегабайт на комнату в предельном случае.
  static constexpr size_t MaxEffectInstancesPerRacer = 4096;
  ...
  //!
  //! ★АРИФМЕТИКА, А НЕ ВКУС — И ПЕРЕСЧИТАНА ПО ОБОИМ ПУТЯМ ВЫДАЧИ (R71-30, находка
  //! ревью 5 #3, WARN). Прежние 512 опирались на довод «каст требует полной звёздной
  //! шкалы». Довод НЕВЕРЕН: шкалу спрашивает только `HandleRequestMagicItem`, а магию
  //! раздаёт ещё и подбор с трассы (`HandleUserRaceItemGet`, `RaceNetworkHandler.cpp`,
  //! ветка `GameMode::Magic`), где никакой шкалы нет. Считаем честный потолок заново,
  //! от того, что ограничивает сервер САМ:
  //!
  //!  1. гонщик держит РОВНО ОДИН магический предмет за раз (`racer.magicItem`
  //!     проверяется и на выдаче, и на касте), значит кастов не больше, чем ПОЛУЧЕНИЙ;
  //!  2. путь подбора: кулдаун на пару «гонщик + экземпляр деки» равен
  //!     `deck.respawnTime`, у всех магических дек (`deckId` 201/202/203) это 3000 мс
  //!     (`courses.yaml`), а экземпляров магических дек на карте не больше ТРЁХ
  //!     (максимум по всем 55 блокам карт того же файла) — не больше 1 получения в
  //!     секунду;
  //!  3. путь шкалы: нужен полный калибр `starPointsMax` = 120 000, а самая крупная
  //!     разовая добавка (золотая подкова) даёт не больше 40 000, то есть ≥ 3 подбора
  //!     скоростной деки с `respawnTime` 500 мс — не больше 1 каста в 1,5 с;
  //!  4. длительность заезда: `timeLimit` в поставляемом `courses.yaml` не больше
  //!     300 с; берём ВДВОЕ больший запас — 600 с (конфиг лежит bind-mount'ом и код
  //!     его не ограничивает);
  //!  5. номеров на каст: не больше ТРЁХ. Не-стена берёт ровно один, а стена — ровно
  //!     столько, сколько у неё сосулек, и с R71-28 их может быть только 1 или 3
  //!     (`IceWallSegmentsNormal/Critical`). ★Раньше здесь стояло «8», потому что
  //!     форма списка не проверялась вовсе.
  //!
  //! Итого честный потолок: (600 x 1 + 600 / 1,5) x 3 = (600 + 400) x 3 = 3000
  //! номеров за заезд. Берём 4096 — ближайшая степень двойки выше честного потолка.
  //!
  //! ★ЗАПРЕТ ОБОРОТА ОТ ЭТОГО НЕ ОСЛАБ. Восемь мест x 4096 = 32 768 номеров, ровно
  //! половина шестнадцатибитного домена (65 535); при рота́ции состава домен не
  //! исчерпать раньше, чем 16 РАЗНЫХ oid'ов выберут бюджет целиком, а `_characterOids`
  //! переиспользует oid вернувшегося игрока — перезаход нового бюджета не даёт.
  //! Запас против прежней редакции уменьшился (было 1/16 домена, стало 1/2), поэтому
  //! фейл-клоузный вопрос о границе домена ниже — не украшение, а рабочий backstop.
  //!
  //! ★ЗАПРЕТ ОБОРОТА ОСТАЁТСЯ НА МЕСТЕ вторым, фейл-клоузным вопросом
  //! (`CanIssueEffectInstances`): арифметика выше делает его недостижимым, но
  //! проверка, которую нельзя достичь, дешевле проверки, которой нет.
