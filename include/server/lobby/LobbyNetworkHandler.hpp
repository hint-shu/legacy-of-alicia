...
private:
  struct ClientContext
  {
    ...
    //! LOA-fix (R74-fix-3, subreview #2, WARN 3): МАСКА БЛОКОВ НАСТРОЕК,
    //! КОТОРЫЕ СЕРВЕР СНЯЛ С КАДРА ВХОДА ЭТОЙ СЕССИИ.
    //!
    //! ★ЗАЧЕМ. Бюджет кадра входа сбрасывает блоки настроек, которые не влезли
    //! (`protocol::BudgetLoginFrame`). Клиент их НЕ ПОЛУЧАЕТ, а его хранилище
    //! настроек заполняется ровно из полученного кадра: разбор клиента
    //! применяет блок только при наличии бита. Стоит игроку сохранить настройки
    //! (диалог макросов проходит по всем восьми слотам и ставит бит
    //! безусловно), как на сервер приезжает ПУСТОЙ блок — и затирает то, что
    //! сервер сам же со своего кадра и снял. Игрок теряет данные, которых он в
    //! глаза не видел, а причина ему не видна.
    //!
    //! Поэтому сессия помнит, что было снято, и ПЕРВОЕ сохранение настроек в
    //! этой сессии не даёт клиенту стереть блок, который до него не доехал.
    //! Флаг снимается по блоку, как только клиент его действительно прислал.
    uint32_t shedSettingsBlocks{0};
  };
