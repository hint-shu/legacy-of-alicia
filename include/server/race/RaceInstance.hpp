...
private:
  ...
  //!
  //! ★ЧЕГО ЛАТЧ НЕ ДЕЛАЕТ. Он не спасает ТАБЛО, если бросок случится в СЕРЕДИНЕ
  //! Stop() — после латча и до рассылки. Это остаётся пред-существующим
  //! поведением (раунд его не ухудшает).
  //!
