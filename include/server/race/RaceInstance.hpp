...
private:
  ...
  //! LOA-fix (R76, backlog #30 этап 1): печатает ЖУРНАЛ ТРАССЫ — по одной
  //! аудит-строке на гонщика и до трёх WARN по порогам раунда 0.
  //! ★ОТДЕЛЬНАЯ ФУНКЦИЯ, А НЕ КУСОК Stop(). Три причины, все проверяемые:
  //! (1) Stop() уже 1400+ строк и его правят соседние раунды R70/R75 — врезка
  //!     внутрь даёт конфликт слияния там, где его можно не иметь;
  //! (2) лесенке раунда нужен НОВЫЙ СИМВОЛ (0/1), иначе доставку правки в
  //!     бинарь пришлось бы мерить только размерами;
  //! (3) функция ничего не мутирует и зовётся ровно один раз — её легко
  //!     прочитать целиком при ревью.
  //! НЕ const: GetRoomUid() неконстантный, а у RaceTracker::GetRacers() есть
  //! ТОЛЬКО неконстантная перегрузка.
  //! ★МЕСТО ВЫЗОВА — НЕ «последней строкой Stop()», А ПЕРЕД БЛОКОМ ДОСТИЖЕНИЙ
  //! R70, И ЭТО ВЫНУЖДЕННАЯ АДАПТАЦИЯ (спека §2.8 писалась до R70).
  //! `tools/round/check_stop_tail.py` машинно держит блок достижений ПОСЛЕДНИМ
  //! оператором Stop(); сам комментарий R70 прямо адресует этот раунд: «встать
  //! ПЕРЕД этим блоком либо доказать, что твой код не бросает». Сделано и то и
  //! другое: вызов стоит перед блоком И обёрнут в собственный catch-олл, потому
  //! что бросок отсюда стоил бы заезду ВСЕХ достижений (латч R75
  //! `_resultsCredited` уже съеден, повторного Stop() не будет).
  //! По существу спеки место не изменилось: вызов по-прежнему стоит ПОСЛЕ всех
  //! начислений и после рассылки баланса — печать не встаёт перед деньгами.
  void LogRaceAudit();
  ...
  //! LOA-fix (R75, #14/#233): ЛАТЧ ИТОГОВ ЗАЕЗДА. `Stop()` подводит итоги ровно
  //! один раз за заезд; повторный вход — no-op со строкой в логе.
  //!
  //! ★ЗАЧЕМ. `Stop()` зовётся из `TickFinishing()` (единственный вызывающий:
  //! grep по src/+include/ даёт RaceInstance.cpp и объявление в этом заголовке),
  //! и раньше строка `_stage = Stage::Waiting;` стояла СРАЗУ ПОСЛЕ вызова в
  //! одном линейном потоке. Любой бросок внутри `Stop()` (`Record::Immutable/
  //! Mutable` на непрогруженной записи бросает std::runtime_error) улетал в
  //! catch в `Tick()`, стадия оставалась `Finishing`, и следующий тик звал
  //! `Stop()` СНОВА: победы, пробег, морковки мести и дейлики начислялись
  //! ВТОРОЙ РАЗ. Комментарии «Идемпотентно: Stop() зовётся ровно один раз за
  //! заезд» держались только на небросающем пути.
  //!
  //! ★ПОЧЕМУ ЛАТЧ НА ИНСТАНСЕ, А НЕ ФЛАГ У ГОНЩИКА И НЕ ПЕРЕСТАНОВКА СТАДИИ.
  //! Флаг у гонщика прикрыл бы только те блоки, которые о нём вспомнят, — это
  //! перечень мест, а не свойство. Перестановка `_stage = Waiting` ПЕРЕД
  //! `Stop()` открывает окно, в котором комната уже «свободна» (GetStage() ==
  //! Waiting гейтит вход и старт), пока `Stop()` ещё ходит по трекеру. Латч же
  //! выражает САМО свойство — «итоги начисляются не более одного раза» — и
  //! покрывает победы, телеметрию, пер-курсовые рекорды, дейлики, месть, смену
  //! мастера и рассылку табло разом, включая любой блок, который допишут завтра.
  //!
  //! ★ЧЕГО ЛАТЧ НЕ ДЕЛАЕТ, СКАЗАНО ПРЯМО. Бросок в СЕРЕДИНЕ `Stop()` — после
  //! латча — теряет ВЕСЬ ХВОСТ функции: табло, смену мастера и блок достижений
  //! R70. Раньше такой бросок «переигрывался» повторным входом, и хвост
  //! доезжал — но ценой ВТОРОГО начисления всего, что стоит выше (это и есть
  //! #233). То есть латч меняет «двойное начисление + доехавший хвост» на
  //! «одинарное начисление + потерянный хвост», и это осознанный размен, а не
  //! недосмотр. Довод (2) у блока достижений в `RaceInstance.cpp` переписан
  //! этим же раундом, чтобы он не утверждал старое правило.
  //!
  //! Сбрасывается в `Start()` рядом с `_raceEpoch` — там же, где кончается
  //! прошлый заезд и начинается новый.
  bool _resultsCredited{false};
