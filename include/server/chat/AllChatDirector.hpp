...
private:
  ...
  //! ВСЕГДА вне замка.
  //!
  //! ★ССЫЛКА ИЗ `GetClientContext` — ПОД ЗАМКОМ У ВСЕХ, КТО ЧИТАЕТ ПОЛЯ
  //! (R80-9, находка ревю #1 correctness-1). Ссылка указывает ВНУТРЬ карты,
  //! поэтому чтение поля через неё — то же обращение к карте, просто
  //! написанное иначе. Оба горячих обработчика потока all-chat
  //! (`HandleChatterChat`, `HandleChatterInputState`) берут `shared_lock`,
  //! КОПИРУЮТ скаляр и отпускают замок ДО первого выхода наружу; дальше
  //! работает копия. Держать замок через `GetCharacter`/`QueueCommand` было бы
  //! ровно тем BLOCK'ом, который получил R78, — поэтому копия, а не удержание.
  //!
  //! ★ЧЕГО ЭТО НЕ ЗАКРЫВАЕТ, СКАЗАНО ПРЯМО: `HandleChatterEnterRoom` по-прежнему
  //! ПРИВЯЗЫВАЕТ ссылку вне замка. Это законно и не гонка: `find` читает
  //! структуру, а СТРУКТУРУ карты (вставка/удаление, то есть рехэш) меняет
  //! ровно один поток — собственный сетевой, он же и есть вход; чужепоточный
  //! писатель правит только ЗНАЧЕНИЯ и записи не двигает. Обе записи через эту
  //! ссылку стоят под исключительным замком. Возврат КОПИИ вместо ссылки и
  //! `*Locked`-путь остаются отдельным раундом (для лобби им был R64-3, #215),
  //! но гонки чтения после R80-9 здесь больше нет.
