...
private:
  ...
  //! с R81 это 86400 с = сутки. Причина знобки — не «настраиваемость ради
  //! настраиваемости»: стенд обязан УВИДЕТЬ протухание, а ждать сутки в каждой
  //! клетке матрицы нельзя.
  //!
  //! ★ЭТО УМОЛЧАНИЕ — КАНАРЕЙКА, А НЕ КОПИЯ НАСТРОЙКИ (LOA-fix R81, #261).
  //! Живое значение приходит из конфига через `SetTtl` (`RanchDirector.cpp`,
  //! `Initialize`) и с R81 равно 86400 с. Здесь СОЗНАТЕЛЬНО оставлено прежнее
  //! короткое 900: если однажды `SetTtl` перестанет вызываться (переименовали
  //! ключ, потеряли строку при рефакторинге), сервер молча возьмёт умолчание —
  //! и мы обязаны УВИДЕТЬ это как протухший попап через 15 минут, а не как
  //! тихий успех. Сделай это умолчание равным настройке — и «SetTtl не позван»
  //! станет НЕОТЛИЧИМО от «SetTtl позван», то есть ровно тем ложно-зелёным,
  //! ради которого R70 писал предупреждение в `Initialize`.
  //! Негатив `neg-f` снимает вызов `SetTtl` и обязан покраснеть на арке
  //! `popup-longhold` именно об эту канарейку.
  ...
  //! LOA-fix (R81, #261): дроссель ВТОРОЙ жалобы удержания — «вытеснен целый
  //! персонаж». ★ОТДЕЛЬНЫЙ ОТ `_achievementHoldOverflowThrottle`, потому что
  //! это другое событие: другой потолок (`CharacterCountCap`, а не
  //! `CharacterCap`), другой виновник и другая строка. Общий дроссель гасил бы
  //! одну жалобу из-за другой (тот же довод — R71-25).
  util::LogThrottle _achievementHoldCharacterEvictionThrottle{std::chrono::seconds(60)};
