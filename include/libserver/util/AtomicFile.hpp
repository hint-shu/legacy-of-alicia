...
//! ★РАДИУС ССЫЛОЧНОГО ИНВАРИАНТА, СКАЗАННЫЙ ЧЕСТНО (решение ведущего по
//! остаточной находке ревью, итерация 7; residual #206). Раунд местами говорил
//! «под `data/` НИКОГДА не ходят по ссылкам» — как о границе безопасности. Это
//! утверждение шире того, что здесь доказано, а завышенная формулировка хуже
//! отсутствующей: по ней принимают решения.
//!
//! Дословная формулировка, которую этот файл держит и которую обязаны
//! повторять спека и RESULTS:
//!
//!   symlinked RECORD files and a symlink in place of a managed directory
//!   entry are refused; symlinks planted by an actor with write access to
//!   data/ are outside the threat model (see #206 residual).
//!
//! ★ПОЧЕМУ ВТОРАЯ ПОЛОВИНА НЕ ДЫРА. Подменить ПОДКАТАЛОГ внутри `data/`
//! (например `data/characters/equipment` -> чужое дерево) может только тот, у
//! кого есть право записи в само дерево данных. У такого актора уже есть
//! возможность удалить, подменить или дописать ЛЮБУЮ запись напрямую — то есть
//! промежуточная ссылка не даёт ему НОВОГО уровня доступа и не является
//! повышением привилегий. Помощники ниже всё равно ходят от дескриптора
//! (`openat`), потому что это дешевле, чем список мест; но продавать это как
//! границу безопасности — нельзя.
#ifndef WIN32
namespace detail
{
...
//!
//! ★И ВЕРДИКТ ОБЯЗАН БЫТЬ ПРОЧИТАН — ОТСЮДА `[[nodiscard]]` (правка ревью,
//! итерация 7). Прежний `std::filesystem::remove(path)` (бросающая перегрузка)
//! ПРОПАГИРОВАЛ отказ: `DataDirector` ловил исключение, возвращал `false`, и
//! `DataStorage` оставлял запись в кэше. Замена на функцию с кодом возврата
//! молча превратила отказ в успех у всех пятнадцати вызывающих: файл оставался
//! на диске, имя объявлялось свободным, а запись «воскресала» после
//! перезапуска. Атрибут делает повторение этой ошибки невозможным молча.
[[nodiscard]] inline bool RemoveManagedFile(
