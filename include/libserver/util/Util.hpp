...
//! LOA-fix (R70, backlog #58): час суток в ИГРОВОМ часовом поясе, 0..23.
//!
//! ★ПОЧЕМУ ФИКСИРОВАННОЕ СМЕЩЕНИЕ, А НЕ СРЕДА. Часовой пояс раунда — решение
//! лида: Europe/Moscow. ПРОВЕРЕНО ЭКСПЕРИМЕНТОМ НА РАНТАЙМ-ОБРАЗЕ (`gcc:15`,
//! 2026-09-02): `std::chrono::current_zone()` читает `/etc/localtime` и
//! переменную окружения `TZ` ИГНОРИРУЕТ — с `TZ=Europe/Moscow` она вернула
//! `Etc/UTC` и местный час, равный UTC. То есть «поставить TZ в compose» —
//! это ТИХО НЕВЕРНАЯ фича, а не фича: шесть достижений раунда — часовые окна,
//! и они уехали бы на три часа, причём стенд согласовался бы с продом и оба
//! были бы неправы относительно игрока.
//! ★Москва живёт на постоянном UTC+03:00 без перехода на летнее время с
//! 26.10.2014, поэтому смещение — константа, а не таблица. Константу нельзя
//! «не доставить» в контейнер.
//! @returns Час 0..23 по игровому часовому поясу.
