...
private:
  ...
  //! Перестроить индекс имён гильдий с диска. Зовётся только из `Initialize` и
  //! потому имеет право БРОСИТЬ: неполный обход каталога гильдий на старте — это
  //! отказ стартовать, а не строка в логе (иначе имя занятой гильдии читалось бы
  //! как свободное).
