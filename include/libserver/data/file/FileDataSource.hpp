...
public:
  ...
  //! То же самое, но с ЯВНЫМ «сейчас» — единственное тело обоих путей.
  //!
  //! ★ЗАЧЕМ ШОВ. Свойство «просьба клиента не умеет опустить
  //! шестидесятисекундный пол» иначе наблюдаемо только реальным ожиданием
  //! минуты, то есть не наблюдаемо в тесте вовсе, — а проверка, которой нет,
  //! это не «свойство держится», это «никто не смотрел». Шов не ослабляет
  //! боевой путь: `TickNameIndexMaintenance` подставляет сюда
  //! `steady_clock::now()` и другого источника времени не имеет.
  void TickNameIndexMaintenanceAt(
    std::chrono::steady_clock::time_point now) noexcept;

  //! Сколько ПЛАНОВЫХ проходов состоялось. Наблюдатель для того же свойства:
  //! «три просьбы за минуту дают ровно один проход, и тот на полу».
  [[nodiscard]] std::size_t ScheduledNameIndexPassCount() const noexcept;

  //! ШВЫ НАБЛЮДАТЕЛЯ ЗА ПУБЛИКАЦИЕЙ ОТМЕТКИ ИНДЕКСА АККАУНТОВ (правка ревью,
  //! итерация 11). Настоящее окно этой находки — чередовка двух потоков;
  //! наблюдать её случайной гонкой значило бы получить тест, который молчит
  //! чаще, чем ловит. Поэтому окно ВОСПРОИЗВОДИТСЯ: тест ставит крючок в точку
  //! бывшего разрыва и пробует опубликовать оттуда `true` НЕБЛОКИРУЮЩЕ —
  //! отказ `try_lock` и есть доказательство, что чередовка невозможна.
  [[nodiscard]] bool TryPublishStaleUserIndexStampForTest(
    std::size_t generation) noexcept;
  [[nodiscard]] bool UserIndexStampValidForTest() const noexcept;
  void SetUserIndexFailureInterleaveHookForTest(void (*hook)()) noexcept;
  //! Объявить, что обход индекса аккаунтов НАЧАЛСЯ, и вернуть его поколение —
  //! ровно то, что делает начало настоящей перестройки. Без этого «опоздавший
  //! обход» в тесте был бы обходом БУДУЩИМ, которому публиковать как раз можно.
  std::size_t BeginUserIndexScanForTest() noexcept;
  //! Пройти путь отказа регистрации имени, не подделывая нехватку памяти.
  void MarkUserIndexFailureForTest() noexcept;
  //! Пройти путь запроса внеочередного ремонта — тот самый, который достижим
  //! именем с провода. Сам он приватен (его зовут пути уникальности), а
  //! свойство «просьба не опускает пол» проверяется именно снаружи.
  void RequestScheduledNameIndexRepairForTest() noexcept;
...
private:
  ...
  //! ПРОСЬБА О ВНЕОЧЕРЕДНОМ ОСМОТРЕ — И ТОЛЬКО ПРОСЬБА (правка ревью,
  //! итерация 11).
  //!
  //! ★ЗАЧЕМ ОТДЕЛЬНЫЙ ФЛАГ, А НЕ СДВИГ ЧАСОВ. Сдвиг часов ОТМЕНЯЛ пол: путь,
  //! достижимый именем с провода, ставил срок в прошлое, и следующий тик
  //! директора (50 Гц) запускал полный обход каталога. Флаг такого права не
  //! даёт по построению: он лишь один из двух поводов, а разрешение по-прежнему
  //! выдаёт `kScheduledNameIndexRepairGap`. Пишут его путь запроса (взводит) и
  //! плановый проход (снимает перед работой); больше никто.
  std::atomic_bool _nameIndexRepairPending{false};

  //! Счётчик состоявшихся плановых проходов — для наблюдателя, см.
  //! `ScheduledNameIndexPassCount`.
  std::atomic_size_t _nameIndexMaintenancePasses{0};
  ...
  //! ★АТОМАРНЫЙ, ЧТОБЫ ЕГО МОЖНО БЫЛО ЧИТАТЬ БЕЗ ЗАМКА на пути промаха; но
  //! ПИШЕТСЯ он ровно одной функцией — `StoreUserIndexStampValidLocked` — и
  //! только под `_userNameIndexMutex` (правка ревью, итерация 11). Пока
  //! писателей было двое, «действительна ли отметка» решалось в двух местах, и
  //! опоздавший обход перезаписывал чужую отметку о неудаче.
  ...
  //! ШОВ НАБЛЮДАТЕЛЯ: зовётся внутри критической секции `MarkUserIndexFailure`
  //! ровно там, где у прежней редакции секция кончалась. В бою пуст.
  std::atomic<void (*)()> _userIndexFailureInterleaveHook{nullptr};
  ...
  //! ЕДИНСТВЕННЫЙ писатель `_userIndexStampValid`; зовётся только с уже взятым
  //! `_userNameIndexMutex`.
  void StoreUserIndexStampValidLocked(
    bool scanWasClean, std::size_t generation) noexcept;
  //! Отметить неудачу регистрации имени: поднять поколение неудач и снять
  //! отметку — ОДНОЙ критической секцией.
  void MarkUserIndexFailure() noexcept;
