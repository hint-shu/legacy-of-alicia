...
//! Отбирает те слоты блока макросов, которые влезают в бюджет, СОХРАНЯЯ ИХ
//! ИНДЕКСЫ. Слот, который в бюджет не влез, ПРОПУСКАЕТСЯ, а не обрывает отбор.
//!
//! ★R74-fix-3 (subreview #2, WARN 1): ПРОПУСК, А НЕ ОБРЫВ. Прежний отбор
//! `break`-ал на первом невлезающем слоте, и пакет с `macros[0]` на 2100 Б и
//! семью крошечными слотами давал НОЛЬ принятых: семь слотов, каждый из которых
//! влез бы, не пробовались вовсе, а хранимый блок затирался восемью пустыми
//! строками — за успешным `AcCmdCLUpdateUserSettingsOK`, то есть молча.
//! Индексы сохраняются законно: `MacroOptions::Write` всегда пишет все восемь
//! строк без счётчика, поэтому пропущенный слот на проводе неотличим от пустого.
//!
//! ★ЖИВЁТ В libserver, А НЕ В ХЕНДЛЕРЕ, РАДИ ПРОВЕРЯЕМОСТИ (NIT 4): цель
//! `alicia-server` в тестовый бинарь не линкуется, поэтому цикл, переписанный
//! в тесте копией, не мог покраснеть ни на одной мутации оригинала.
//!
//! @param requested Присланный клиентом блок.
//! @param accepted Получает отобранные слоты (остальные — пустые строки).
//! @returns Сколько слотов принято.
[[nodiscard]] std::size_t SelectMacroSlotsWithinBudget(
