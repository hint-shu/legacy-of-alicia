...
//! ПОТОЛОК ДАННЫХ ОДНОГО ПАКЕТА У КЛИЕНТА, В БАЙТАХ.
//!
//! ★R74-fix-3 (subreview #2, WARN 5): БЮДЖЕТ КАДРА ЦЕЛИТСЯ В КЛИЕНТА, А НЕ В
//! СВОЙ БУФЕР. `util::MaxCommandDataSizeBytes` (8192) — это буфер СЕРВЕРА, а
//! не свойство того, кто кадр читает. Собственный разбор проекта кладёт
//! максимум данных пакета клиента на 7168
//! (`research/discord/soa-studio/transcripts/reverse-engineering.txt:3375-3382`:
//! «7168 must be the max packet data size», и 7182 с двенадцатибайтовым
//! заголовком плюс два байта). Из двух названных там чисел берём МЕНЬШЕЕ:
//! ошибиться в эту сторону значит отдать на 14 байт меньше, ошибиться в другую
//! — отдать кадр, который клиент не прочтёт.
//!
//! Серверный буфер остаётся ВЕРХНЕЙ границей: он физически ограничивает
//! запись, а этот потолок — цель, в которую метит сброс.
//!
//! ★ЖИВЫМ КЛИЕНТОМ ЭТО НЕ ПОДТВЕРЖДЕНО (владелец объявил паузу, Wine-клиент не
//! поднимался) — записано остатком раунда, а не выдано за замер.
constexpr std::size_t MaxClientPacketDataBytes = 7168;
