...
//! LOA-fix (R74-fix-3, subreview #2, NIT 3): ПОТОЛОК ЭКИПИРОВКИ ГОНЩИКА.
//!
//! Число не новое: ровно оно жило отдельным литералом в обработчике входа в
//! комнату (`RaceNetworkHandler.cpp`, кламп ростера) — то есть ЧЕТВЁРТЫЙ
//! сериализатор того же хранимого `characterEquipment` знал свой потолок, а
//! сам писатель — нет. Тезис раунда «один хранимый список — один потолок»
//! доводится и здесь: константа переезжает в заголовок протокола, и её берут
//! обе стороны.
