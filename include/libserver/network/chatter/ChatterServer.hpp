...
public:
  ...
  //! LOA-fix (R80-0, round80, backlog #235): вызывается на КАЖДУЮ входящую
  //! порцию данных клиента, ДО разбора заголовка команды.
  //!
  //! ★ЗАЧЕМ НА ЭТОМ УРОВНЕ, А НЕ В ХЕНДЛЕРЕ. Команда без зарегистрированного
  //! обработчика уходит в ветку «Unhandled chatter command» и хуком уровнем
  //! выше не видна вовсе, а пакет всё равно доказывает, что пир жив. Битый
  //! заголовок и вовсе бросает (`Invalid chatter header`) и уносит управление
  //! мимо любой строки ниже. Это дословно тот же довод, которым R21-3b
  //! поставил такой же крючок в `CommandServer` — и та же форма.
  //!
  //! ★Дефолт — no-op: `PrivateChatDirector` наследует тот же интерфейс, и
  //! чисто виртуальный метод сломал бы ему сборку ради службы, которой раунд
  //! не касается.
  //! ★Реализация обязана быть дешёвой и НЕ БРОСАТЬ: исключение отсюда уходит
  //! в сетевой read-loop и рвёт клиенту соединение.
  //! @param clientId ID клиента (в дефолтной пустышке не именован — иначе
  //!        -Wunused-parameter на каждом наследнике).
  virtual void HandleClientActivity(network::ClientId) {}
};
