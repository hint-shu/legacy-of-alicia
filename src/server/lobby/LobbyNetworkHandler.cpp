...
void LobbyNetworkHandler::HandleClientDisconnected(ClientId clientId)
{
      ...
      && clientContext.characterUid != data::InvalidUid)
    {
      ...
      if (revoked)
      {
        // LOA-fix (R80-5, round80, backlog #235): ВМЕСТЕ С МЕССЕНДЖЕРСКИМИ
        // ЗАКРЫВАЕМ И ALL-CHAT-СЕССИИ ПЕРСОНАЖА.
        //
        // ★ГЕЙТ ТОТ ЖЕ, ЧТО У R78, И ЭТО НЕ ЛЕНЬ. `revoked == true` означает,
        // что снятый ключ был ТЕКУЩИМ, то есть эта лобби-сессия не вытеснена
        // более свежей. Опоздавшая уборка старой сессии не имеет права рвать
        // чат-сокеты игрока, который уже перезашёл, — это вернуло бы #255
        // другой дорогой.
        //
        // ★СВОЙ `RunCleanupStep` И СТОИТ ПЕРВЫМ. Пояс на шаг нужен затем,
        // чтобы бросок одной службы не съедал уборку другой; а стоит он до
        // мессенджерского зова потому, что тот пояса на себе НЕ имеет (он
        // внутри общего шага «messenger key revoke»), и обратный порядок дал
        // бы мессенджеру право унести all-chat с собой.
        //
        // ★ЯРУС 1 ПОКРЫВАЕТ НЕ ВСЁ, и это записано вслух: если ключ
        // мессенджера не выдавался (клиент не дошёл до `GetMessengerInfo`)
        // или уже перевыдан (`revoked == false`), зов не случится вовсе.
        // Именно поэтому ярус 2 (развёртка) — несущая часть, а не украшение:
        // он подберёт такие сокеты через `orphanGrace` после того, как
        // персонаж уйдёт из игры.
        util::RunCleanupStep(
          "all chat session teardown",
          clientId,
          [&]()
          {
            _serverInstance.GetAllChatDirector().CloseSessionsOfCharacter(
              clientContext.characterUid);
          });
