...
void RaceInstance::Stop()
{
  ...
  try
  {
    ...
    //      один вопрос — куда доставить уведомление (снимок `clientIds` ниже).
    ...
    for (const auto& [characterUid, racer] : racers)
    {
      ...
      // игру до `Stop()`, теряет только УВЕДОМЛЕНИЕ (снимок `clientIds` его не
      // найдёт), а прогресс обязан быть записан — он его заработал.
      if (not participated(racer))
        continue;
